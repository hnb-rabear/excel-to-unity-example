...
namespace RCore.Framework.Data
{
	...
	public class AttributesCollection<T> where T : AttributeParse
	{
		...
		public bool IsEqual<M>(AttributesCollection<M> pOther) where M : AttributeParse
		{
			for (int i = 0; i < Attributes.Count; i++)
			{
				bool sameId = false;
	...
	public class AttributeParse
	{
		...
		public float max;
		//=== Optional
		public float[] values; //we may have more than one value, for example, value[0] for Gold and value[1] for Gem.
		public float[] increases; //Sometimes, Increase is defined by multiple values, for example, increase[0] for level and increase[1] for rarity.
		public float[] unlocks; //Unlock can have multiple conditions, for example, level, and rarity
		public float[] maxes; //Max can be defined for different values, for instance, the max value when rarity equals 1 or the max value when rarity equals 2.
		...
		public virtual string GetDescription(int pLevel = 1, int pNextLevel = 1, int pValueIndex = -1)
		{
			...
			if (pNextLevel != pLevel)
			{
				...
				if (nextValue != curValue)
					str = $"{Round(curValue, 2)} (<color=#299110>+{Round(nextValue - curValue, 2)}</color>)";
				else
        ...
        private static float Round(float pValue, int pDecimal)
        {
