	...
	public class ModsContainer
	{
		...
		protected virtual void AddModAndValues(int id, params float[] values)
		{
			if (mods.ContainsKey(id))
			{
				for (int i = 0; i < values.Length; i++)
					mods[id][i] = mods[id][i] + values[i];
			}
			else
			{
				var temp = new float[values.Length];
				Array.ConstrainedCopy(values, 0, temp, 0, values.Length);
				mods.Add(id, temp);
			}
		}
		public void AddMods(Dictionary<int, float[]> pMods)
		{
			...
			foreach (var mod in pMods)
			{
				...
				newMods.Add(new Mod(mod.Key, mod.Value));
			}
			onModsChanged?.Invoke(newMods);
		}
		...
		//---- REMOVE
		protected virtual void RemoveModAndValues(int id, float[] values)
		{
			if (mods.ContainsKey(id))
			{
		...
		public bool RemoveCircleMod(int pSourceId, int pModId)
		{
			for (int i = 0; i < circleMods.Count; i++)
			{
				if (circleMods[i].sourceId == pSourceId && circleMods[i].id == pModId)
