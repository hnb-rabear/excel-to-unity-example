namespace Excel2Unity.Basic
{
	...
	public static class ExampleLocalization
	{
		...
		public static readonly Dictionary<string, string> languageDict = new Dictionary<string, string>() {  { "english", "ExampleLocalization_english" }, { "spanish", "ExampleLocalization_spanish" }, { "japan", "ExampleLocalization_japan" }, { "chinese", "ExampleLocalization_chinese" }, { "korean", "ExampleLocalization_korean" }, { "thai", "ExampleLocalization_thai" }, };
	    ...
	    public static void Init()
		{
			...
			{
	#if UNITY_EDITOR
				Debug.Log($"Init {nameof(ExampleLocalization)}");
	#endif
	...
	#if UNITY_EDITOR
	        var localizedText = pObj.GetComponent<ExampleLocalizationText>();
	        if (localizedText != null)
	        {
	            Debug.LogError($"{pObj.name} should not have ExampleLocalizationText!");
	    ...
	    public class DynamicText
	    {
	        ...
	        public void Refresh()
	        {
	            if (curLangIndex != ExampleLocalization.curLangIndex)
	            {
	#if UNITY_EDITOR
	                var text = Get(key);
	                try
	                {
						...
						curLangIndex = ExampleLocalization.curLangIndex;
	#if UNITY_EDITOR
	                }
	                catch (Exception ex)
	                {
		...
		public static List<string> FindUsingIDs(string pPath)
		{
			...
			foreach (var guid in assetIds)
			{
				...
				var components = obj.GetComponentsInChildren<ExampleLocalizationText>(true);
		...
		public static void CreateClassContainsRefinedIds(List<string> pRefinedIds)
		{
			...
			{
				enumIds += $"{nameof(ExampleLocalization)}.ID.{pRefinedIds[i]}";
				intIds += $"{nameof(ExampleLocalization)}.{pRefinedIds[i]}";
			...
			string path = $"{directoryPath}/{nameof(ExampleLocalization)}RefinedIds.cs";
		...
		{
			string content = $"namespace {typeof(ExampleLocalization).Namespace}\n"
				+ "{\n"
				+ "\tusing System.Collections.Generic;\n"
				+ $"\tpublic class ExampleLocalizationRefinedIds\n"
				+ "\t{\n"
				+ "\t\tpublic static readonly List<ExampleLocalization.ID> enumIds = new List<ExampleLocalization.ID> { ENUM_IDS };\n"
				+ "\t\tpublic static readonly List<int> intIds = new List<int> { INT_IDS };\n"
				+ "\t}\n"
				+ "}";
			return content;
		}
		[UnityEditor.MenuItem("Assets/Create/RCore/Localization Refined Ids/Create ExampleLocalizationRefinedIds")]
		public static void CreateExampleLocalizationRefinedIds()
		{
	...
	public class ExampleLocalizationGetter
	{
	    ...
	    public ExampleLocalizationGetter(string pKey, string pDefault)
	    {
	        key = pKey;
	        defaultStr = pDefault;
	        ...
	        ExampleLocalization.Get(key, ref mIndex);
	        mCheckKey = true;
	#endif
	    }
	    ...
	    public string Get()
	    {
	        ...
	        if (mIndex == -1)
	            return defaultStr;
	        var text = ExampleLocalization.Get(mIndex).ToString();
	        if (string.IsNullOrEmpty(text))
	            return defaultStr;
	        else
	            return ExampleLocalization.Get(mIndex).ToString().Replace("\\n", "\u000a");
	    }
	    ...
	    public static string GetCached(string pKey)
	    {
	        if (cachedTexts.ContainsKey(pKey))
	        {
	            ...
	            if (id != -1)
	            {
	                string text = ExampleLocalization.Get(cachedTexts[pKey]).ToString();
	        ...
	        {
	            int id = -1;
	            string text = ExampleLocalization.Get(pKey, ref id).ToString();
